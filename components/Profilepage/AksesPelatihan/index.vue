        ...
        <b-jumbotron class="shadow-none bg-gray">
          ...
          <b-container v-if="pelatihans.length > 0" class="mt-5">
            ...
            <b-row v-else class="row justify-content-start">
              <b-col
                v-if="listIndex <= pelatihans.length"
                ...
                <mdb-card>
                  ...
                  <mdb-card-body>
                    <mdb-badge class="mb-2 badge__category shadow-none">{{
                      pelatihans[listIndex - 1].status_pendaftaran_value
                    }}</mdb-badge>

                    <mdb-card-title class="mb-5">{{
                      pelatihans[listIndex - 1].kegiatan_title
                    }}</mdb-card-title>
                    <mdb-card-text
                    ...
                    <span style="font-size: 12px; margin-top: 1.5rem"
                      ><i
                        class="fa fa-calendar fa-fw fa-lg"
                        aria-hidden="true"
                      ></i>
                      {{
                        $moment(pelatihans[listIndex - 1].tanggal_awal).format(
                          "LL"
                        )
                      }}
                      -
                      {{
                        $moment(pelatihans[listIndex - 1].tanggal_akhir).format(
                          "LL"
                        )
                      }}</span
                    >
            ...
            <b-row
              v-if="listToShow <= pelatihans.length"
  ...
  data() {
    return {
      ...
      header: {
        ...
        context:
          "Silahkan mulai belajar dengan klik list pelatihan yang telah kamu beli, atau lihat pelatihan terbaru lainnya dibagian bawah halaman ini. Selamat belajar!",
      },
    };
  },
    ...
    .card {
      ...
      .content__card-event {
        ...
        .truncate {
          ...
          --is-single-line: 1 -
            Clamp(0, Calc(var(--line-clamp) - 1), var(--line-clamp));
