<template>
	<div>
		<mdb-row class="justify-content-start pembayaran__card">
			<mdb-col col="12"  lg="8" xs="10" sm="12" class="mb-3">
				<b-card no-body class="overflow-hidden shadow-none">
					<b-row v-if="$device.isDesktop" no-gutters class="mt-2 row justify-content-start ml-2 rincian__event-table">
						<h5 style="margin-left:.5rem;">Ringkasan Belanja Paket Membership</h5>
						<table class="table table-borderless">
							<thead>
								<tr>
									<th scope="row">
										<b>Nama Paket</b>
									</th>
									<th scope="row">
										<b>Subtotal</b>
									</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>
										{{data_paket.paket_nama}}
									</td>

									<td>
										{{$format(data_paket.harga)}}
									</td>
								</tr>
							</tbody>
						</table>
						<b-col md="12" xs="12" sm="12">
							<div class="dropdown-divider"></div>
						</b-col>
						<b-col md="5" xs="12" sm="12">
							<b-card title="Total Harga" class="shadow-none rincian__bayar">
								<b-card-text>
									<h2> {{$format(data_paket.harga)}} </h2>
								</b-card-text>
							</b-card>
						</b-col>
					</b-row>
					<b-row v-else no-gutters class="mt-2 row justify-content-start rincian__event-table mb-3">
						<h5>Ringkasan Belanja</h5>
						<table class="table table-borderless">
							<thead>
								<tr>
									<th scope="row">
										<b>Nama Paket</b>
									</th>
									<th scope="row">
										<b>Subtotal</b>
									</th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>
										{{data_paket.paket_nama}}
									</td>

									<td>
										{{$format(data_paket.harga)}}
									</td>
								</tr>
							</tbody>
						</table>
						<b-col md="12" xs="12" sm="12">
							<div class="dropdown-divider"></div>
						</b-col>
						<b-col md="5" xs="12" sm="12">
							<b-card title="Total Harga" class="shadow-none rincian__bayar">
								<b-card-text>
									<h2> {{$format(data_paket.harga)}} </h2>
								</b-card-text>
							</b-card>
						</b-col>
					</b-row>
				</b-card>
			</mdb-col>
		</mdb-row>
		<mdb-row  :class="`${$device.isDesktop ?  'row justify-content-end check__point' : 'row justify-content-center check__point'}`">
			<mdb-col col="12" lg="4" sm="12">
				<b-card class="shadow-none list__bank">
					<b-card-text>
						<div class="input__bank">
							
							<b-container>
								<div v-if="data_paket.bank_nama == 'BCA'">
									
									<b-card no-body class="shadow-none overflow-hidden card__bank-list">
										<b-row no-gutters>
											<b-col md="4" class="mt-5">
												<b-card-img :src="require('~/assets/images/bank/logo-bank-bca.svg') " alt="Image" class="rounded-0"></b-card-img>
											</b-col>
											<b-col md="8">
												<b-card-body title="Transfer Bank">
													<b-card-text>
														<h6 class="mt-2">Bank {{data_paket.bank_nama}} </h6>
														<p> a.n : <span class="text-capitalize">{{data_paket.nama_rek}}</span> </p>

														<h5>
															{{data_paket.no_rek}}
														</h5>
													</b-card-text>
												</b-card-body>
											</b-col>
										</b-row>
									</b-card>
								</div>
								<div v-else>
									<b-card no-body class="shadow-none overflow-hidden card__bank-list">
										<b-row no-gutters>
											<b-col md="4" class="mt-5">
												<b-card-img :src="require('~/assets/images/bank/logo-bank-mandiri.svg')" alt="Image" class="rounded-0"></b-card-img>
											</b-col>
											<b-col md="8">
												<b-card-body title="Transfer Bank">
													<b-card-text>
														<h6 class="mt-2">Bank {{data_paket.bank_nama}} </h6>
														<p> a.n : <span class="text-capitalize">{{data_paket.nama_rek}}</span> </p>

														<h5>
															{{data_paket.no_rek}}
														</h5>
													</b-card-text>
												</b-card-body>
											</b-col>
										</b-row>
									</b-card>
								</div>
								<div v-else>
									<small>No Bank History</small>
								</div>
								<div class="dropdown-divider mt-2"></div>
							</b-container>
						</div>
						<mdb-col lg="12" xs="12" sm="12" class="col__card-upload-file mt-2 mb-2">


							<div v-if="data_paket.status_daftar === 'Menunggu Konfirmasi Admin'" class="form-group">
								<mdb-row class="row justify-content-center">
									<mdb-col md="12" class="mt-2 mb-2">
										<b-badge pill variant="primary">Bukti Bayar Anda</b-badge>
									</mdb-col>
									<mdb-col md="12" class="mb-4">
										<img :src="data_paket.bukti_bayar" width="150" class="img-fluid">
									</mdb-col>

									<mdb-col md="12">
										<mdb-btn disabled class="btn my__btn-primary rounded-pill btn-block shadow-none"> {{data_paket.status_daftar }} </mdb-btn>
									</mdb-col>

									<mdb-col md="12" class="mt-3">
										<nuxt-link :to="`/profile/${username}/events`" class="btn btn-primary rounded-pill btn-block shadow-none"> Selesai <mdb-icon icon="arrow-right" /> </nuxt-link>
									</mdb-col>

								</mdb-row>
							</div>

							<div v-else class="form-group mt-3">
								<div v-if="data_paket.status_bayar" class="form-group">
									<mdb-alert color="info">
										{{data_paket.status_bayar}}
									</mdb-alert>
								</div>
							</div>

						</mdb-col>
					</b-card-text>

				</b-card>
			</mdb-col>
		</mdb-row>
	</div>
</template>


<script>

	export default{
		props: ['token', 'api_url'],

		data(){
			return {
				data_paket: {},
				username: null
			}
		},

		mounted(){
			this.statusPembayaran(),
			this.userData()
		},

		methods: {
			statusPembayaran(){
				const url = `${this.api_url}/web/paket-membership-user/${this.$route.params.id}`
				this.$axios.defaults.headers.common.Authorization = `Bearer ${this.token.accessToken}`
				this.$axios.get(url)
				.then(({data}) => {
					console.log(data)
					this.data_paket = data.paket
				})
				.catch(err => console.error(err))
			},

			userData(){
				const url = `${this.api_url}/web/user`
				this.$axios.defaults.headers.common.Authorization = `Bearer ${this.token.accessToken}`
				this.$axios.get(url)
				.then(({data}) => {
					console.log(data)
					this.username = this.$username(data.user.nama)
				})
				.catch(err => console.error(err))
			}
		}
	}
</script>