<template>
  ...
  <div v-else>
    <mdb-row :class="`${$device.isMobile ? 'row justify-content-center' : ''}`">
      ...
      <mdb-col lg="6" xs="6" sm="12" col="12" class="event__info">
        ...
        <mdb-row class="mt-3" col="12">
            ...
            <div v-else>
              ...
              <mdb-row class="row justify-content-start">
                <mdb-col md="12" col="12" sm="12">
                  <mdb-btn @click="BeliKelas(details.kegiatan_id, details)" class="btn my__btn-secondary rounded-pill btn-block mb-2" :size="`${$device.isDesktop ? 'md' : 'sm'}`">
                   <mdb-icon far icon="credit-card" size="lg"/>&nbsp; {{status_pendaftaran == "Daftar" ? "Beli Kelas" : status_pendaftaran}}
                  </mdb-btn>
                </mdb-col>
         ...
         <mdb-col v-else-if="status_pendaftaran == 'Menunggu Konfirmasi'" md="12" xs="12" sm="12" class="mt-5">
          ...
          <div v-else>
            <mdb-btn @click="MenungguKonfirmasi(details.kegiatan_id)" class="btn my__btn-primary rounded-pill btn-block shadow-none" :size="`${$device.isDesktop ? 'md' : 'sm'}`"> 
              <mdb-icon icon="calendar-alt" :size="`${$device.isDesktop ? 'lg' : 'sm'}`"/> {{status_pendaftaran == 'Menunggu Konfirmasi' ? 'Check Konfirmasi Pendaftaran' : status_pendaftaran}}
            </mdb-btn>
          </div>
        </mdb-col>
        ...
        <mdb-col v-else-if="status_pendaftaran == 'Menunggu Pembayaran'" md="12" xs="12" sm="12" class="mt-5">
          ...
          <div v-else>
            ...
            <mdb-btn @click="CheckPembayaran(details.kegiatan_id)" class="btn my__btn-secondary rounded-pill btn-block shadow-none" :size="`${$device.isDesktop ? 'md' : 'sm'}`"> 
              <mdb-icon icon="check" :size="`${$device.isDesktop ? 'lg' : 'sm'}`"/> {{status_pendaftaran == 'Menunggu Pembayaran' ? 'Menunggu Pembayaran' : status_pendaftaran}}
            </mdb-btn>
          </div>
        </mdb-col>
        ...
        <mdb-col v-else md="12" xs="12" sm="12" class="mt-5">
          ...
          <div v-else>
            <mdb-btn @click="ProfileEvent($username(profiles.nama), data_event.event_id, $slug(details.kegiatan_title))" v-if="data_event || token.accessToken" class="btn my__btn-primary rounded-pill btn-block shadow-none" :size="`${$device.isDesktop ? 'md' : 'sm'}`">
              <h5>Lanjut Pelatihan <mdb-icon icon="chalkboard-teacher" size="lg"/> </h5>
            </mdb-btn>
          </div>
      ...
      <!-- Beli dengan paket membership -->
      <mdb-row class="mt-5 row__paket-membership">
        <mdb-col v-if="loadingPaket" md="12" xs="12" sm="12">
    ...
    methods: {
      ...
      pilihPaket(){
        ...
        setTimeout(() => {
          ...
          this.aktifPaket = !this.aktifPaket
        }, 1500)
      },
